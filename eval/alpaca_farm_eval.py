    ...
    if args.openai_engine is None:
        ...
        with torch.inference_mode():
            for samples in tqdm(dataloader):
                ...
                input_texts = [convert_to_msg_format(input, instruction) for input, instruction in
                               zip(inputs, instructions)]
                ...
                for input_string, instruction, output in zip(samples['input'], samples['instruction'], outputs):
                    ...
                    my_outputs.append(
                        {"instruction": instruction, "input": input_string, "generator": f"{model_name}-greedy-long",
        ...
        for samples in tqdm(dataloader):
            inputs, instructions = samples['input'], samples['instruction']
            input_texts = [
                {"prompt": convert_to_msg_format(input, instruction), "id": "tmp"} for input, instruction in
                zip(inputs, instructions)
            ]
            ...
            my_outputs += [{
                "instruction": instructions[i], "input": inputs[i], "generator": f"{model_name}-greedy-long",
                "output": r["output"]
