...
@torch.no_grad()
def generate_completions(model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, disable_tqdm=False,
                         **generation_kwargs):
    generations = []
    if not disable_tqdm:
            ...
            if stop_id_sequences:
                for output_idx in range(batch_outputs.shape[0]):
                    for token_idx in range(batch_input_ids.shape[1], batch_outputs.shape[1]):
                        if any(batch_outputs[output_idx, token_idx: token_idx + len(stop_sequence)].tolist()
                               == stop_sequence for stop_sequence in stop_id_sequences):
...
def get_next_word_predictions(model, tokenizer, prompts, candidate_token_ids=None, batch_size=1,
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        batch_logits = model(**tokenized_prompts).logits[:, -1, :]
...
def load_hf_lm_and_tokenizer(
    ...
    if "bloom" in tokenizer_name_or_path:
