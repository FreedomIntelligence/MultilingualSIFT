...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    train_dataset = dataset_cls(tokenizer=tokenizer, data_path=data_args.train_data_path)
    eval_datasets = {}
    eval_data_paths = data_args.val_data_path.split(",")
    for eval_data_path in eval_data_paths:
        name = eval_data_path.split("/")[-2] + "-" + eval_data_path.split("/")[-1].split(".")[0]
        eval_datasets[name] = dataset_cls(tokenizer=tokenizer, data_path=eval_data_path)
    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer)
    return dict(train_dataset=train_dataset,
                eval_dataset=None if len(eval_datasets) == 0 else eval_datasets,
